...
for i, score in enumerate(score_input):
    if current_slot == 10:
        current_score += get_score(score) + get_score(score_input[i + 1])

        if score_input[i + 1] == '/':
            current_score -= get_score(score)

        if i + 2 < len(score_input):
            current_score += get_score(score_input[i + 2])

            if score_input[i + 2] == '/':
                current_score -= get_score(score_input[i + 1])

        score_card[current_slot] = current_score
        break

    if score == 'X':
        current_score += 10 + ((get_score(score_input[i + 1]) + get_score(
            score_input[i + 2])) if score_input[i + 2] != '/' else 10)
        score_card[current_slot] = current_score
        current_slot += 1
        sub_slot = 1
        continue
    ...
    if score == '/':
        current_score += 10 + get_score(
            score_input[i + 1]) - get_score(score_input[i - 1])
        score_card[current_slot] = current_score
        current_slot += 1
        sub_slot = 1
        continue
...
if interested >= 1 and interested <= 10:
    print(
        score_card[interested] - (score_card[interested - 1]
                                  if interested > 1 else 0))
else:
    print(score_card[10])
